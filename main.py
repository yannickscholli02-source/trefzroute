...
async def export_routenliste(request: Request):
    try:
    ...
    print(f"[DEBUG] export_routenliste body size: {len(body)} bytes")
    try:
        data = json.loads(body)
    except Exception as e:
        print(f"[DEBUG] JSON parse error: {e}")
        raise HTTPException(status_code=400, detail=f'Ungültiger JSON-Body: {e}')

    raw = b64lib.b64decode(data['file_base64'])
    filename = data.get('filename', 'routenliste.xls')
    route_lookup = data.get('route_lookup', {})
...
# -- CHUNK UPLOAD FÜR GROSSE DATEIEN --
import tempfile, uuid, os as _os

_upload_chunks: dict = {}  # upload_id -> {chunks: [], filename: str, total: int}
...
class ProcessRoutenlisteChunked(BaseModel):
    upload_id: str
    route_lookup: dict
    depot: str = ''

@app.post('/upload_chunk_init')
async def upload_chunk_init(req: ChunkUploadInit):
    uid = str(uuid.uuid4())
    _upload_chunks[uid] = {'chunks': [None] * req.total_chunks, 'filename': req.filename, 'total': req.total_chunks, 'received': 0}
    return {'upload_id': uid}

@app.post('/upload_chunk')
async def upload_chunk(req: ChunkData):
    import base64 as b64lib
    if req.upload_id not in _upload_chunks:
        raise HTTPException(status_code=404, detail='Upload nicht gefunden')
    entry = _upload_chunks[req.upload_id]
    entry['chunks'][req.chunk_index] = b64lib.b64decode(req.data_base64)
    entry['received'] += 1
    return {'received': entry['received'], 'total': entry['total']}

@app.post('/process_routenliste_chunked')
async def process_routenliste_chunked(req: ProcessRoutenlisteChunked):
    if req.upload_id not in _upload_chunks:
        raise HTTPException(status_code=404, detail='Upload nicht gefunden')
    entry = _upload_chunks[req.upload_id]
    if entry['received'] < entry['total']:
        raise HTTPException(status_code=400, detail=f'Nur {entry["received"]}/{entry["total"]} Chunks erhalten')

    raw = b"".join(entry['chunks'])
    filename = entry['filename']
    del _upload_chunks[req.upload_id]
    ...
    if filename.lower().endswith('.xls') and not filename.lower().endswith('.xlsx'):
        try:
            import xlrd
            xls_wb = xlrd.open_workbook(file_contents=raw, formatting_info=True)
            wb = openpyxl.Workbook()
            wb.remove(wb.active)
            for si in range(xls_wb.nsheets):
                xls_ws = xls_wb.sheet_by_index(si)
            ...
            raise HTTPException(status_code=500, detail=f'XLS-Fehler: {e}')
    else:
        wb = load_workbook(io.BytesIO(raw))

    ws = wb.worksheets[0]
    route_lookup = req.route_lookup

    NEW_COLS = ['Route', 'Lfd. Nr.', 'Zusatz', 'Zustellung', 'Uhrzeit 1', 'Uhrzeit 2', 'Fahrer']
    N = len(NEW_COLS)

    # Spalten-Indizes in Originaldatei finden (1-basiert)
    strasseCol = plzCol = ortCol = None
    for ci in range(1, ws.max_column + 1):
        h = str(ws.cell(row=1, column=ci).value or '').strip()
        if re.search(r'straße|strasse|postfach', h, re.I): strasseCol = ci
        elif re.match(r'^plz$', h, re.I): plzCol = ci
        elif re.match(r'^ort$', h, re.I): ortCol = ci

    # Spalten nach A (col 1) einfügen — alle bestehenden Spalten nach rechts verschieben
    ws.insert_cols(2, N)

    # Header-Styling von Spalte A kopieren als Vorlage
    HEADER_BG = '305496'
    BORDER_CLR = 'BFBFBF'
    thin = Side(style='thin', color=BORDER_CLR)
    def brd(): return Border(left=thin, right=thin, top=thin, bottom=thin)
    ...
    for i, col_name in enumerate(NEW_COLS):
        cell = ws.cell(row=1, column=2+i, value=col_name)
        # Stil von A1 kopieren falls vorhanden
        a1 = ws.cell(row=1, column=1)
        if a1.font: cell.font = copy(a1.font)
        if a1.fill and a1.fill.patternType: cell.fill = copy(a1.fill)
        else: cell.fill = PatternFill('solid', fgColor=HEADER_BG)
        cell.border = brd()
        cell.alignment = Alignment(wrap_text=True, vertical='center', horizontal='center')
        ws.column_dimensions[get_column_letter(2+i)].width = [10,10,10,14,12,12,12][i]

    def norm_addr_py(s):
        return re.sub(r'\s+', ' ', str(s).lower().strip())

    # Nach dem Insert: Spaltenindizes für Straße/PLZ/Ort anpassen (+N)
    if strasseCol: strasseCol += N
    if plzCol:     plzCol     += N
    if ortCol:     ortCol     += N

    matched = 0
    for ri in range(2, ws.max_row + 1):
        if not strasseCol: continue
        strasse = str(ws.cell(row=ri, column=strasseCol).value or '').strip()
        plz = ws.cell(row=ri, column=plzCol).value if plzCol else ''
        plz_str = str(int(float(plz))) if isinstance(plz, (int, float)) else str(plz or '').strip()
        ort = str(ws.cell(row=ri, column=ortCol).value or '').strip() if ortCol else ''
        if not strasse: continue

        key = norm_addr_py(f'{strasse}, {plz_str} {ort}')
        entry = route_lookup.get(key)

        vals = [entry.get('routeId','') if entry else '', entry.get('stopNum','') if entry else '',
                '', '', entry.get('zfFrom','') if entry else '', entry.get('zfTo','') if entry else '', '']
        if entry: matched += 1

        # Styling von der Zeile kopieren (Spalte A als Vorlage)
        ref_cell = ws.cell(row=ri, column=1)
        for i, val in enumerate(vals):
            cell = ws.cell(row=ri, column=2+i, value=val if val != '' else None)
            try:
                if ref_cell.font: cell.font = copy(ref_cell.font)
                if ref_cell.fill and ref_cell.fill.patternType: cell.fill = copy(ref_cell.fill)
            except: pass
            cell.border = brd()
            cell.alignment = Alignment(vertical='center', horizontal='center' if i < 2 else 'left')

    print(f'[INFO] {matched} Zeilen gematcht')
    out_buf = io.BytesIO()
    wb.save(out_buf)
    out_buf.seek(0)
    out_name = filename.rsplit('.', 1)[0] + '_Routen.xlsx'
