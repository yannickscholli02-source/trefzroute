...
class ExcelExportRequest(BaseModel):
    ...
    depot: str
    single_route_id: Optional[int] = None  # wenn gesetzt, nur diese Route exportieren
...
def build_route_sheet(wb, r: ExcelRoute, depot: str):
    ...
    # Abfahrt
    depot_row = [('Abfahrt', False), (depot, False), (dep, False), ('—', False), ('— (Abfahrt)', False), ('—', False)]
    for ci, (val, _) in enumerate(depot_row, 1):
    ...
    # Rückkehr
    ret_row = 7 + len(r.stops)
    ret_data = [f'{r.id}/{len(r.stops)+1}', depot, ret, '—', '— (Rückkehr)', '—']
    for ci, val in enumerate(ret_data, 1):
        cell = ws.cell(row=ret_row, column=ci, value=val)
        cell.font = depot_font()
        cell.fill = depot_fill()
        cell.border = brd()
        cell.alignment = wrap_align(center=(ci == 1))
...
async def export_excel(req: ExcelExportRequest):
    ...
    for r in routes_to_export:
        build_route_sheet(wb, r, req.depot)
