...
class MatrixOptimizeRequest(BaseModel):
    ...
    dur_matrix: List[List[float]]   # bereits inkl. Abladezeit
    max_stops_per_route: int = 999
    max_duration_s: Optional[float] = None  # wird ignoriert, Zeitfenster übernehmen
...
def solve_ortools_vrp(dist_matrix, dur_matrix, depot_idx=0):
    ...
    if not sol:
        # Retry mit mehr Fahrzeugen
        print("[INFO] Kein Ergebnis, erhöhe Fahrzeuganzahl...")
    ...
    if ORTOOLS_OK:
        try:
            ...
            for v in range(num_vehicles):
                ...
                # Abfahrtszeit: OR-Tools Wert, aber mindestens so berechnen:
                # erster Stopp Ankunftszeit minus Fahrzeit depot→erster Stopp
                # → so spät wie möglich, frühestens 06:00
                start_idx = routing.Start(v)
                departure_s_raw = sol.Value(time_dim.CumulVar(start_idx))
                # Rückwärts vom ersten Stopp berechnen für exakte Minute
                ...
                for i, node in enumerate(route_indices):
                    ...
                    stops_out.append(StopResult(
                        ...
                        leg_dur_s=leg_dur_s,
                        leg_dur_text=format_dur(leg_dur_s),
            ...
            for ri, route in enumerate(greedy_routes):
                ...
                for node, arrival_s in route:
                    ...
                    if departure_s is None:
                        departure_s = max(EARLIEST_DEPARTURE, arrival_s - leg_dur_s)
                    route_dist += leg_dist_m
                    route_dur  += leg_dur_s
        ...
        for ri, route in enumerate(greedy_routes):
            ...
            for node, arrival_s in route:
                ...
                stops_out.append(StopResult(
                    ...
                    leg_dur_s=leg_dur_s, leg_dur_text=format_dur(leg_dur_s),
                    arrival_time=format_time(arrival_s),
                ))
                prev = node
            route_dist += dist_matrix[prev][0]
