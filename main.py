...
async def export_routenliste(req: RoutenlisteRequest):
    try:
    ...
    # Decode file
    raw = b64lib.b64decode(req.file_base64)
    buf = io.BytesIO(raw)

    # Load workbook — xlrd for .xls, openpyxl for .xlsx
    filename = req.filename.lower()
    if filename.endswith('.xls') and not filename.endswith('.xlsx'):
        try:
            import xlrd
            xls_wb = xlrd.open_workbook(file_contents=raw, formatting_info=True)
            from xlrd2xlwt_compat import xls_to_xlsx
        except:
            # Fallback: try openpyxl directly (sometimes works)
            pass
        # Convert via xlrd → openpyxl manually
        try:
            import xlrd
            ...
            for si in range(xls_wb.nsheets):
                ...
                out_ws = out_wb.create_sheet(title=xls_ws.name)
                for ri in range(xls_ws.nrows):
                    for ci in range(xls_ws.ncols):
                        cell_val = xls_ws.cell_value(ri, ci)
                        out_ws.cell(row=ri+1, column=ci+1, value=cell_val)
            wb = out_wb
        except Exception as e:
    ...
    for ri in range(2, ws.max_row + 1):
        ...
        entry = req.route_lookup.get(key)
