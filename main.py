...
async def process_routenliste_chunked(req: ProcessRoutenlisteChunked):
    ...
    def safe_copy_style(src_cell, dst_cell):
        try:
            if src_cell.font: dst_cell.font = _copy(src_cell.font)
        except: pass
        try:
    ...
    for ci in range(1, ws.max_column + 1):
        ...
        if re_mod.search(r'straße|strasse|postfach', h, re_mod.I): strasseCol = ci
        elif re_mod.match(r'^plz$', h, re_mod.I): plzCol = ci
        elif re_mod.match(r'^ort$', h, re_mod.I): ortCol = ci
    ...
    def norm_addr_py(s):
        return re_mod.sub(r'\s+', ' ', str(s).lower().strip())
